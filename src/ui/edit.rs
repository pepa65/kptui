...
fn field_help_items(slim_mode: bool, multiline: bool) -> &'static [&'static str] {
	if slim_mode {
		if multiline { &["[^s]", "[Esc]"] } else { &["[Enter]", "[Esc]"] }
	} else {
		if multiline { &["[^s] Save field", "[Esc] Cancel"] } else { &["[Enter] Save field", "[Esc] Cancel"] }
	}
}
...
pub fn draw_edit(frame: &mut Frame, app: &mut App) {
	...
	let password_view = if password_is_set { Some(FieldView::new(Zeroizing::new(entry.password.clone()))) } else { None };
	...
	let help_items = if editing_field { field_help_items(app.slim_mode, selected == NOTES_INDEX) } else { nav_help_items(app.slim_mode) };
	...
	let items = vec![
		compact_field(NAME_INDEX, "Name", name, vec![], true),
		compact_field(USER_INDEX, "User", user, vec![], true),
		compact_field(PASSWORD_INDEX, "Password", String::new(), vec![], !password_is_set),
		compact_field(URL_INDEX, "URL", url, vec![], true),
		compact_field(TOTP_INDEX, "TOTP", String::new(), totp_extra, totp_raw.is_empty()),
		compact_field(LAST_MODIFIED_INDEX, "Modified", last_modified, vec![], true),
	];
	...
	// Render the password separately from the List.
