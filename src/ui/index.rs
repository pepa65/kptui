...
fn masked_password<'a>(entry: &'a Entry, theme: &Theme) -> Line<'a> {
	...
	if entry.password_reuse_count > 1 {
		spans.push(Span::styled(format!(" [{}]", entry.password_reuse_count), warning));
	}
...
fn masked_user<'a>(entry: &'a Entry, theme: &Theme) -> Line<'a> {
	...
	if entry.duplicate_user_count > 1 {
		spans.push(Span::styled(format!(" [{}]", entry.duplicate_user_count), warning));
	}
...
fn slim_row<'a>(entry: &'a Entry, theme: &Theme) -> Line<'a> {
	...
	if entry.duplicate_user_count > 1 {
		spans.push(Span::styled(format!(" u[{}]", entry.duplicate_user_count), warning));
	}

	if entry.password_reuse_count > 1 {
		spans.push(Span::styled(format!(" p[{}]", entry.password_reuse_count), warning));
	}
