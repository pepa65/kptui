...
pub fn import_csv(path: &Path) -> anyhow::Result<Vec<Entry>> {
	let mut reader = csv::ReaderBuilder::new().flexible(true).from_path(path).with_context(|| format!("couldn't open {}", path.display()))?;
	...
	let get = |record: &csv::StringRecord, idx: Option<usize>| -> String { idx.and_then(|i| record.get(i)).unwrap_or("").trim().to_string() };
...
pub fn import_json(path: &Path) -> anyhow::Result<Vec<Entry>> {
	...
	let flat: Vec<FlatEntry> = serde_json::from_str(&text).context("couldn't parse JSON: expected a Bitwarden-style export or a flat array of entries")?;
...
impl From<BitwardenItem> for Entry {
	fn from(item: BitwardenItem) -> Self {
		...
		Entry {
			...
			url: login.uris.into_iter().find_map(|u| u.uri).unwrap_or_default(),
